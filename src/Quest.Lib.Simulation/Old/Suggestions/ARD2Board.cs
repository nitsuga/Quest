...
namespace Quest.Lib.Suggestions
{
    ...
    public class ARD2Board
    {
        ...
        /// </summary>
        public List<Lib.IncidentView> Incidents;

        /// <summary>
        /// a list of resources
        /// </summary>
        public List<Lib.ResourceView> Resources;
        ...
        {
            // get resource
            ResourceView res = (from r in Resources where r.Callsign == am.Resource.Callsign select r).FirstOrDefault();
            IncidentView inc = (from i in Incidents where i.Incidentid == am.Incident.Incidentid select i).FirstOrDefault();
        ...
        List<IMove> CalculateStandbyAssignmentMoves(ResourceView r)
        {
            ...
            {
                bool rethrow = ExceptionPolicy.HandleException(ex, "TracePolicy");
                if (rethrow)
                    throw;
        ...
        List<IMove> CalculateIncidentAssignmentMoves()
        {
            ...
            try
            {
                ...
                if (waiting_incs.Count() > 0)
                    foreach (IncidentView inc in waiting_incs)
                    {
                        int frucount = 0;
                        int ambcount = 0;
